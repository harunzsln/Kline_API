...
class KlineGenerator:
    ...
    def generate(start_date_str: str, end_date_str: str):
        ...
        print(
            f"KlineGenerator: Generating data between {start_date_str} and {end_date_str}..."
        )

        # 1. Step: Create a date range for 1-hour intervals
        kline_starts = pd.to_datetime(
            pd.date_range(start=start_date_str, end=end_date_str, freq="1h")
        )
        ...
        price_changes = 1 + np.random.randn(num_points) / 10

        prices = 100 * price_changes.cumprod()

        # 3. Step: Create Open and Close DataFrames
        data = {"open": prices[:-1], "close": prices[1:]}
...
class KlineReceiver:
    ...
    def fetch(self, start_date_str: str, end_date_str: str):
        ...
        self.df_1h = self._generator.generate(start_date_str, end_date_str)
        print(f"Fetch completed. {len(self.df_1h)} rows of 1-hour data fetched.")

    def get_1h_klines(self):
        print("\nKlineReceiver: 'get_1h_klines' method is being executed...")
        if self.df_1h.empty:
            print(
                "Warning: No raw data available for processing. Please run the 'fetch' method first."
            )
            return pd.DataFrame()
    ...
    def get_4h_klines(self):
        ...
        if self.df_1h.empty:
            print(
                "Warning: No 1-hour data available for processing. Please run the 'fetch' method first."
            )
            return pd.DataFrame()

        df_4h = self.df_1h.resample("4h").agg({"open": "first", "close": "last"})
    ...
    def get_1d_klines(self):
        ...
        df_1d = self.df_1h.resample("1D").agg({"open": "first", "close": "last"})
